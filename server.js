// AutoCaptio backend â€” the ONLY place your API key ever lives.
...
// and settings changes are REAL and visible from any device â€” not just
// stuck in one browser's memory.
//
// âš ï¸ Note: Render's free tier has no persistent disk, so this file resets
// whenever the server redeploys or restarts (not on every request â€” it
...
const uploadProjectVideo = multer({
  ...
  limits: { fileSize: 60 * 1024 * 1024 } // 60MB cap â€” keeps this workable on a free server
});
...
// Keeps a rolling "recent activity" feed for the admin dashboard â€” not a
...
function applyApprovedPlanToUser(email, planName){
  ...
  if(!user){
    // Payment was made under an email with no signed-up account yet â€” create
...
// the server (set in Render â†’ Environment, never committed to GitHub, never
// sent to the browser). The password is checked here on the server; the
// browser only ever gets back a random, meaningless session token â€” never
...
function requireAdmin(req, res, next){
  ...
  if(!token || !adminSessions.has(token)){
    return res.status(401).json({ error: 'Not authorized â€” please log in as admin again' });
  }
  next();
}
...
app.get('/', (req, res) => {
  res.send('AutoCaptio backend is running âœ“');
});
...
app.post('/transcribe', upload.single('file'), async (req, res) => {
  try {
    ...
    if (!apiKey) {
      return res.status(500).json({ error: 'Server is missing GROQ_API_KEY â€” add it in your hosting dashboard\'s environment variables' });
    }
    ...
    if(req.body.language) form.append('language', req.body.language); // e.g. 'hi', 'en' â€” improves accuracy when known
...
// ============ SETTINGS (UPI ID, QR, prices) â€” admin-only to change ============
...
app.post('/orders/:id/approve', requireAdmin, (req, res) => {
  ...
  logActivity(`Approved ${order.plan} (â‚¹${order.amount}) for ${order.email || order.user}`);
  // NOTE: real email confirmation to the user needs an email-sending service
  // (Resend/SendGrid) wired up â€” not done yet, see project notes. The
...
// âš ï¸ Render's free tier disk is NOT persistent across redeploys/restarts â€”
...
app.post('/projects', uploadProjectVideo.single('video'), (req, res) => {
  try{
    const { email, title, captionsJson, presetKey } = req.body;
    if(!email) return res.status(400).json({ error: 'Not signed in' });
    const db = loadDB();
    const project = {
...
app.get('/projects', (req, res) => {
  const db = loadDB();
  const mine = db.projects.filter(p => p.email === req.query.email);
  res.json(mine.map(p => ({ ...p, hasVideo: !!p.videoFile })));
});
app.get('/projects/:id/video', (req, res) => {
  const db = loadDB();
...
app.delete('/projects/:id', (req, res) => {
  ...
  if(project.email !== req.body.email) return res.status(403).json({ error: 'Not your project' });
...
// separately â€” this is a genuine, working email+password account system in
...
// tier the server sleeps after inactivity, so this isn't a guaranteed clock â€”
...
app.post('/me/consume-credit', (req, res) => {
  const db = loadDB();
...
// language), we ask a free Groq LLM to translate the caption text â€” same
...
app.post('/translate', async (req, res) => {
  try{
    ...
    const languageInstruction = targetLanguage === 'hinglish'
      ? 'Hinglish (write Hindi words using plain English/Latin letters, the way Indians casually text â€” NOT Devanagari script)'
      : targetLanguage;

    const prompt = `Translate the following transcript into ${languageInstruction}. Keep the SAME NUMBER of words as natural as possible so timing stays aligned â€” do not add explanations, only output the translated transcript text, nothing else.\n\nTranscript: "${originalText}"`;
    ...
    // possible â€” not perfectly synced to the new word boundaries, but a
...
// Google directly whether it's genuine â€” no extra library/dependency needed.
...
app.post('/auth/google', async (req, res) => {
  try{
    ...
    if(!clientId) return res.status(500).json({ error: 'Server is missing GOOGLE_CLIENT_ID â€” Google Sign-In is not set up yet' });
